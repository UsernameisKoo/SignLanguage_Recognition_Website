...
for file in os.listdir(data_dir):
    if file.endswith(".pickle"):
        with open(os.path.join(data_dir, file), "rb") as f:
            ...
            # 피처 수 확인 및 변환 (84피처로 확장되었는지 확인)
            feature_data = np.array(gesture_data["data"])
            if feature_data.shape[1] != 84:
                raise ValueError(f"데이터 피처 수가 84가 아닙니다: {feature_data.shape[1]}")
...
print("교차 검증을 거친 Voting 앙상블 모델이 'voting_cross_validated.p'에 저장되었습니다.")
